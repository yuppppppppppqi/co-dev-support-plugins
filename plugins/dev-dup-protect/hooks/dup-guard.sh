#!/usr/bin/env bash
# Duplicate Development Guard — PreToolUse hook.
...
TTL_SECONDS=2700
MARKER_DIR="$HOME/.claude/.dev-dup-protect"
...
cat <<EOF
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "Duplicate Development Guard: no duplicate check has been recorded for this project in the last 45 minutes, so Linear issue creation is blocked. Before registering a backlog item you must: (1) run the duplicate-detector subagent (dev-dup-protect plugin) on the feature being registered — it searches the Linear backlog and GitHub branches/PRs for the same or similar work; (2) show its report (alert level, similar issues with status/assignee, similar branches) to the user; (3) if the user still wants to register, run: \\"$plugin_root/scripts/mark-checked.sh\\" and then retry this Linear tool call. The /dev-dup-protect:dup-check and /dev-dup-protect:backlog-add commands perform this whole flow. Do not run mark-checked.sh without actually performing the check."
  }
}
EOF
exit 0
